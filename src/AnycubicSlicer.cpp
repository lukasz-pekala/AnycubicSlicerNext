...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in AnycubicSlicerNext before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in AnycubicSlicerNext before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in AnycubicSlicerNext before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in AnycubicSlicerNext before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in AnycubicSlicerNext before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in AnycubicSlicerNext before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in AnycubicSlicerNext before uploading."},
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on AnycubicSlicerNext before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest AnycubicSlicerNext."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    set_current_thread_name("AnycubicSlicer_main");
    ...
    /*#if !defined(_WIN32) && !defined(__APPLE__)
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch AnycubicSlicerNext GUI soon";
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current AnycubicSlicerNext Version %1%")%SoftFever_VERSION;
    ...
    if (export_to_3mf) {
        ...
        if (need_regenerate_thumbnail || need_regenerate_no_light_thumbnail || need_regenerate_top_thumbnail) {
            ...
            if (ret == GLFW_FALSE) {
                ...
                GLFWwindow* window = glfwCreateWindow(640, 480, "AnycubicSlicerNext", NULL, NULL);
                if (window == NULL)
                {
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into AnycubicSlicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a AnycubicSlicer process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the AnycubicSlicerNext process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"AnycubicSlicerNext is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using AnycubicSlicer.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes AnycubicSlicerNext "
...
extern "C" {
    __declspec(dllexport) int __stdcall AnycubicSlicer_main(int argc, wchar_t **argv)
    {
