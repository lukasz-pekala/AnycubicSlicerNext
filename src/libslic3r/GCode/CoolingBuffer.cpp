...
std::vector<PerExtruderAdjustments> CoolingBuffer::parse_layer_gcode(const std::string &gcode, std::vector<float> &current_pos) const
{
    ...
    for (size_t i = 0; i < m_extruder_ids.size(); ++ i) {
        ...
        // Anycubic: To enable dont slow down external perimeters feature per filament (extruder)
    ...
    // Anycubic: Whether we had our first extrusion in this layer.
    ...
    for (; *line_start != 0; line_start = line_end) 
    {
        ...
        if (line.type) {
            ...
            // Anycubic: only slow down movements since the first extrusion
            if (boost::contains(sline, ";_EXTRUDE_SET_SPEED"))
                layer_had_extrusion = true;
            
            // Anycubic: Dont slowdown external perimeters for layer time feature
            ...
            // Anycubic: Dont slowdown external perimeters for layer time works by not marking the external perimeter as adjustable, 
        ...
        } else if (boost::starts_with(sline, ";_INTERNAL_BRIDGE_FAN_START")) { // Anycubic: Add support for separate internal bridge fan speed control
            line.type = CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START;
        } else if (boost::starts_with(sline, ";_INTERNAL_BRIDGE_FAN_END")) { // Anycubic: Add support for separate internal bridge fan speed control
        ...
        // Anycubic: For any movements before this layer's first ever extrusion, we exclude them from the layer time calculation.
    ...
    std::vector<PerExtruderAdjustments>    &per_extruder_adjustments)
{
    ...
    bool internal_bridge_fan_control= false; // Anycubic: Add support for separate internal bridge fan speed control
    int  internal_bridge_fan_speed   = 0; // Anycubic: Add support for separate internal bridge fan speed control
...
#define EXTRUDER_CONFIG(OPT) m_config.OPT.get_at(m_current_extruder)
        ...
        if (int(layer_id) >= close_fan_the_first_x_layers) {
            ...
            if( internal_bridge_fan_speed < 0 ) { // Anycubic: Backwards compatibility setting for Anycubic internal bridge fan speed setting - if set at -1 (which is the default) use the overhang fan speed settings.
...
#undef EXTRUDER_CONFIG
            ...
            internal_bridge_fan_control = false; // Anycubic: Add support for separate internal bridge fan speed control
            internal_bridge_fan_speed = 0; // Anycubic: Add support for separate internal bridge fan speed control
        }
    ...
    // Anycubic: Reduce set fan commands by deferring the GCodeWriter::set_fan calls. Inspired by SuperSlicer
    ...
    std::unordered_map<int, bool> fan_speed_change_requests = {{CoolingLine::TYPE_OVERHANG_FAN_START, false},
                                                               {CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START, false}, // Anycubic: Add support for separate internal bridge fan speed control
    ...
    for (const CoolingLine *line : lines) {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL) {
            ...
            need_set_fan = true;
        } else if (line->type & CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START) { // Anycubic: Add support for separate internal bridge fan speed control
        ...
        } else if (line->type & CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_END) { // Anycubic: Add support for separate internal bridge fan speed control
            if (internal_bridge_fan_control && fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START]) {
        ...
        if (need_set_fan) {
            ...
            } else if (fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START]){ // Anycubic: Add support for separate internal bridge fan speed control
