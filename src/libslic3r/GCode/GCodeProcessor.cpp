...
//Anycubic: External device purge tag
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: AnycubicSlicer is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::AnycubicSlicer, SLIC3R_APP_NAME },
    { EProducer::AnycubicSlicer, "generated by AnycubicSlicer" },
    { EProducer::AnycubicSlicer, "generated by BambuStudio" },
    { EProducer::AnycubicSlicer, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by AnycubicSlicer,
        // extract the config from it
        if (m_producer == EProducer::AnycubicSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from AnycubicSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    // Anycubic: Integrate filament consumption for purging performed to an external device and controlled via macros
    ...
    if (!producers_enabled || m_producer == EProducer::AnycubicSlicer) {
        // height tag
        ...
        // Anycubic: manual tool change tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::AnycubicSlicer: { return process_bambuslicer_tags(comment); }
...
void  GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line)
{
    ...
    // Anycubic: we now use spiral_vase_layers for proper layer detect when scarf joint is enabled,
...
void GCodeProcessor::run_post_process()
{
    ...
    public:
        ...
        void insert_lines(const Backtrace& backtrace, const std::string& cmd,
            ...
            // Anycubic: find start pos by seaching G28/G29/PRINT_START/START_PRINT commands
    ...
    auto process_placeholders = [&](std::string& gcode_line) {
        ...
        if (line.length() > 1) {
            ...
            if (true &&
                ...
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
                    ...
                    if (mode == PrintEstimatedStatistics::ETimeMode::Normal || machine.enabled) {
                        char buf[128];
                        if (!s_IsBBLPrinter)
                            // Anycubic: compatibility with klipper_estimator
            ...
            // Anycubic: write total layer number, this is used by Bambu printers only as of now
        ...
        if (cmd.size() >= 2) {
            ...
            export_lines.insert_lines(
                ...
                [tool_number, this](unsigned int id, const std::vector<float>& time_diffs) {
                    ...
                    // Anycubic: M104.1 for XL printers, I can't find the documentation for this so I copied the C++ comments from
