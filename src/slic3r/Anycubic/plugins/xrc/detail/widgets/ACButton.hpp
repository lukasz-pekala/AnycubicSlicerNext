...
public:
	...
	void SetStdStringIcon(const std::string& iconName);

	void SetStdStringInactiveIcon(const std::string& iconName);

	void SetStdStringHoverIcon(const std::string& iconName);
    ...
    void SetConfigWizardIndex() { m_isConfigWizardIndex = true; }
	...
	void setIconSize(const wxSize& size) { m_iconSize = size; Rescale(); }

    void setIcoAtLeft() { m_icoAtLeft = true; }
    wxSize GetszContentSize() { return m_szContent; }
    void   setDropDownStyle(bool index,int type=0);
    bool   GetDropDownIndex() { return m_isDropIndex; }
    void   setDropDownClick(bool index) { 
        m_isDropIndex = index; 
    }

    void   SetLeftTopText(int num, wxColour text_color)
    {
        left_topNum   = num;
        left_topColor = text_color;
    }
...
private:
    ...
    //wxSize m_offset;
    bool                        m_isDropDownIndex{false};
    bool                        m_isDropIndex{false};
    ScalableBitmap active_icon;
    ScalableBitmap inactive_icon;
    ScalableBitmap hover_icon;
    std::vector<ScalableBitmap> m_dropDownScalableBitmapList;
    ...
    bool                  m_isConfigWizardIndex{false};
};
