    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    wxGetApp().add_anycubic_window(wxASCII_STR("filament"), p->m_panel_filament_title);
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update AnycubicSlicerNext or restart AnycubicSlicerNext to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
                ...
                Semver             file_version;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by AnycubicSlicerNext, load geometry data only."), _L("Load 3mf"));
                    }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on AnycubicSlicerNext(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
