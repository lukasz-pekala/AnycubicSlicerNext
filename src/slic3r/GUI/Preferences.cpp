...
wxBoxSizer* PreferencesDialog::create_item_link_association(wxWindow* parent, wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with AnycubicSlicer so that Anycubic can open models from") + " " + website_name;
...
wxWindow* PreferencesDialog::create_general_page()
{
    ...
    auto item_single_instance = create_item_checkbox(_L("Allow only one AnycubicSlicer instance"), page, 
    #if __APPLE__
    ...
    #else
            _L("If this is enabled, when starting AnycubicSlicer and another instance of the same AnycubicSlicer is already running, that instance will be reactivated instead."), 
    #endif
            50, "single_instance");
    ...
    auto item_remember_printer_config = create_item_checkbox(_L("Remember printer configuration"), page, _L("If enabled, AnycubicSlicer will remember and switch filament/process configuration for each printer automatically."), 50, "remember_printer_config");
    ...
    auto item_multi_machine = create_item_checkbox(_L("Multi-device Management (Take effect after restarting AnycubicSlicer)."), page, _L("With this option enabled, you can send a task to multiple devices at the same time and manage multiple devices."), 50, "enable_multi_machine");
...
#ifdef _WIN32
    auto title_associate_file = create_item_title(_L("Associate files to AnycubicSlicer"), page, _L("Associate files to AnycubicSlicer"));

    // associate file
    auto item_associate_3mf  = create_item_checkbox(_L("Associate .3mf files to AnycubicSlicer"), page,
                                                        _L("If enabled, sets AnycubicSlicer as default application to open .3mf files"), 50, "associate_3mf");
    auto item_associate_stl  = create_item_checkbox(_L("Associate .stl files to AnycubicSlicer"), page,
                                                        _L("If enabled, sets AnycubicSlicer as default application to open .stl files"), 50, "associate_stl");
    auto item_associate_step = create_item_checkbox(_L("Associate .step/.stp files to AnycubicSlicer"), page,
                                                         _L("If enabled, sets AnycubicSlicer as default application to open .step files"), 50, "associate_step");

    auto title_associate_url = create_item_title(_L("Associate web links to AnycubicSlicer"), page, _L("Associate URLs to AnycubicSlicer"));
