...
struct CurlGlobalInit {
  ...
  CurlGlobalInit() {
    ...
    if (CURLcode ec = ::curl_global_init(CURL_GLOBAL_DEFAULT); ec != CURLE_OK) {
      message +=
          "CURL initialization failed. See the log for additional details.";
...
struct form_file {
  ...
  form_file(fs::path const &p,
            const boost::filesystem::ifstream::off_type offset,
            const size_t content_length)
      : ifs(p, std::ios::in | std::ios::binary), init_offset(offset),
...
struct Http::priv {
  ...
  // Using a deque here because unlike vector it doesn't ivalidate pointers on
  // insertion
  ...
  static int xfercb(void *userp, double dltotal, double dlnow, double ultotal,
                    double ulnow);
  static int xfercb_legacy(void *userp, double dltotal, double dlnow,
                           double ultotal, double ulnow);
  static size_t form_file_read_cb(char *buffer, size_t size, size_t nitems,
                                  void *userp);
  static size_t headers_cb(char *buffer, size_t size, size_t nitems,
                           void *userp);

  void set_timeout_connect(long timeout);
  void set_timeout_max(long timeout);
  void form_add_file(const char *name, const fs::path &path,
                     const char *filename,
                     boost::filesystem::ifstream::off_type offset,
                     size_t length);
  /* mime */
...
static int log_trace(CURL *handle, curl_infotype type, char *data, size_t size,
                     void *userp) {
#ifndef NDEBUG
...
Http::priv::priv(const std::string &url)
    : curl(::curl_easy_init()), form(nullptr), form_end(nullptr), mime(nullptr),
      headerlist(nullptr), error_buffer(CURL_ERROR_SIZE + 1, '\0'), limit(0),
      cancel(false) {
  ...
  ::curl_easy_setopt(curl, CURLOPT_URL,
                     url.c_str()); // curl makes a copy internally
  ...
  // if the file is uploaded in packets, it will cause the upload time to be
  // longer
...
bool Http::priv::ca_file_supported(::CURL *curl) {
  ...
  if (::curl_easy_getinfo(curl, CURLINFO_TLS_SSL_PTR, &tls) == CURLE_OK) {
    if (tls->backend == CURLSSLBACKEND_SCHANNEL ||
        tls->backend == CURLSSLBACKEND_DARWINSSL) {
...
int Http::priv::xfercb(void *userp, double dltotal, double dlnow,
                       double ultotal, double ulnow) {
  auto self = static_cast<priv *>(userp);
...
int Http::priv::xfercb_legacy(void *userp, double dltotal, double dlnow,
                              double ultotal, double ulnow) {
  return xfercb(userp, dltotal, dlnow, ultotal, ulnow);
}

size_t Http::priv::form_file_read_cb(char *buffer, size_t size, size_t nitems,
                                     void *userp) {
  ...
  try {
    ...
    if (f->content_length == 0) {
      ...
      max_read_size =
          std::min(max_read_size, size_t(f->content_length - read_size));
      f->ifs.read(buffer, max_read_size);
    }
...
size_t Http::priv::headers_cb(char *buffer, size_t size, size_t nitems,
                              void *userp) {
  auto self = static_cast<priv *>(userp);
...
void Http::priv::form_add_file(const char *name, const fs::path &path,
                               const char *filename,
                               boost::filesystem::ifstream::off_type offset,
                               size_t length) {
  // We can't use CURLFORM_FILECONTENT, because curl doesn't support Unicode
  // filenames on Windows and so we use CURLFORM_STREAM with boost ifstream to
  // read the file.
  ...
  if (filename != nullptr) {
    ::curl_formadd(&form, &form_end, CURLFORM_COPYNAME, name, CURLFORM_FILENAME,
                   filename, CURLFORM_CONTENTTYPE, "application/octet-stream",
...
void Http::priv::set_post_body(const std::string &body) { postfields = body; }
...
void Http::priv::set_del_body(const std::string &body) { postfields = body; }
...
std::string Http::priv::curl_error(CURLcode curlcode) {
  return (boost::format("curl:%1%:\n%2%\n[Error %3%]") %
          ::curl_easy_strerror(curlcode) % error_buffer.c_str() % curlcode)
      .str();
}

std::string Http::priv::body_size_error() {
  return (boost::format("HTTP body data size exceeded limit (%1% bytes)") %
          limit)
      .str();
}
  ...
  if (res != CURLE_OK) {
    if (res == CURLE_ABORTED_BY_CALLBACK) {
      if (cancel) {
        ...
        bool cancel = true;
        if (progressfn) {
          progressfn(dummyprogress, cancel);
        }
      } else {
        // The abort comes from the CURLOPT_READFUNCTION callback, which means
        // reading file failed
        if (errorfn) {
          errorfn(std::move(buffer), "Error reading file for file upload", 0);
        }
      }
    } else if (res == CURLE_WRITE_ERROR) {
      if (errorfn) {
        errorfn(std::move(buffer), body_size_error(), 0);
      }
    } else {
      if (errorfn) {
        errorfn(std::move(buffer), curl_error(res), 0);
      }
    };
  } else {
    long http_status = 0;
    ...
    // BBS check error http status code
      ...
      if (errorfn) {
        errorfn(std::move(buffer), std::string(), http_status);
      }
    }
  }
}
...
Http &Http::timeout_connect(long timeout) {
  if (timeout < 1) {
    timeout = priv::DEFAULT_TIMEOUT_CONNECT;
  }
  if (p) {
    p->set_timeout_connect(timeout);
  }
  return *this;
}
...
Http &Http::timeout_max(long timeout) {
  if (timeout < 1) {
    timeout = priv::DEFAULT_TIMEOUT_MAX;
  }
  if (p) {
    p->set_timeout_max(timeout);
  }
  return *this;
}
...
Http &Http::form_add(const std::string &name, const std::string &contents) {
  if (p) {
    ::curl_formadd(&p->form, &p->form_end, CURLFORM_COPYNAME, name.c_str(),
...
Http &Http::form_add_file(const std::string &name, const fs::path &path,
                          ...
                          size_t length) {
  if (p) {
    p->form_add_file(name.c_str(), path.c_str(), nullptr, offset, length);
  }
  return *this;
}
...
Http &Http::mime_form_add_text(std::string &name, std::string &value) {
  if (p) {
    p->mime_form_add_text(name.c_str(), value.c_str());
  }
  return *this;
}
...
Http &Http::mime_form_add_file(std::string &name, const char *path) {
  if (p) {
    p->mime_form_add_file(name.c_str(), path);
  }
  return *this;
}
...
Http &Http::form_add_file(const std::wstring &name, const fs::path &path,
                          ...
                          size_t length) {
  if (p) {
    p->form_add_file((char *)name.c_str(), path.c_str(), nullptr, offset,
                     length);
  }
  return *this;
}
...
Http &Http::form_add_file(const std::string &name, const fs::path &path,
                          ...
                          size_t length) {
  if (p) {
    p->form_add_file(name.c_str(), path.c_str(), filename.c_str(), offset,
                     length);
  }
  return *this;
}
...
Http::Ptr Http::perform() {
  ...
  if (self->p) {
    auto io_thread = std::thread([self]() { self->p->http_perform(); });
...
Http Http::get(std::string url) { return Http{std::move(url)}; }
...
std::ostream &operator<<(std::ostream &os, const Http::Progress &progress) {
  os << "Http::Progress("
     << "dltotal = " << progress.dltotal << ", dlnow = " << progress.dlnow
     << ", ultotal = " << progress.ultotal << ", ulnow = " << progress.ulnow
     << ")";
  return os;
}
